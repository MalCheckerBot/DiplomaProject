...
def check_link(url):
    ...
    gsb_message = "☁️ Google Safe Browsing:\n" + (
        gsb_result["error"] if "error" in gsb_result else "Ссылка безопасна ✅" if gsb_result["is_safe"] else f"Найдены угрозы: {', '.join(gsb_result['threats'])} ⚠️")
...
def check_with_spamcalls(phone_number):
    try:
        ...
        proxies = {
            "http": "http://kvufzuvo:k703tsxis9nt@38.154.227.167:5868",
            "https": "http://kvufzuvo:k703tsxis9nt@38.154.227.167:5868"
        }
...
def check_phone_message(message):
    ...
    bot.send_message(message.chat.id, "☎️ Жду от тебя номер.\n\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
...
def handle_button(message):
    ...
    if message.text == "🔗 Проверить ссылку":
        ...
        bot.send_message(user_id, "☎️ Жду от тебя номер.\n\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
...
def handle_text_message(message):
    ...
    if state == 'start':
        ...
        if message.text in ["Да✅", "/yes"]:
            ...
            if "error" in result:
                bot.send_message(user_id, result + "\n☎️ Введи номер заново или ⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
                user_state[user_id] = 'awaiting_phone'
            else:
            ...
            user_state[user_id] = 'awaiting_phone'
            bot.send_message(user_id, "☎️ Введи номер заново.\n\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
