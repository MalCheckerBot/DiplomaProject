...
def check_link(url):
    ...
    vt_message = "🦠 VirusTotal:\n" + (vt_result["error"] if "error" in vt_result else f"Угрозы: {vt_result['malicious']}/{vt_result['total_engines']} ⚠️\nОтчёт: {vt_result['report_url']}")

    sa_result = check_with_scamadviser(url)
    sa_message = "☄️ ScamAdviser:\n" + (sa_result["error"] if "error" in sa_result else f"Уровень доверия: {sa_result['trust_score']} / 100 ⭐\nОтчёт: {sa_result['report_url']}")

    gsb_result = check_with_google_safe_browsing(url)
    gsb_message = "☁️ Google Safe Browsing:\n" + (
        gsb_result["error"] if "error" in gsb_result else "Ссылка безопасна ✅" if gsb_result["is_safe"] else f"Найдены угрозы ⚠️: {', '.join(gsb_result['threats'])}")
    ...
    if is_dangerous:
        result += "\n\n📝 Итог:\n🔗 Данная ссылка - Потенциально Вредоносная ⚠️\n❗️ Не рекомендую её открывать ❗️"
    elif is_safe:
...
def check_phone(phone_number):
    spam_result = check_with_spamcalls(phone_number)
    if "error" in spam_result:
        return spam_result["error"]
    else:
        if spam_result["is_spam"]:
            return f"⚠️ Номер {phone_number} отмечен как спам или мошенничество.\n🔎 Детали:\n{spam_result['details']}\n📝 Отчёт: {spam_result['report_url']}"
        else:
            return f"✅ Номер {phone_number} безопасен.\n📝 Отчёт: {spam_result['report_url']}"
            ...
            while True:
                ...
                if analysis_response.status_code == 200:
                    ...
                    if status == 'completed':
                        ...
                        return {"error": f"⚠️ Ошибка, статус анализа: {status}"}
                else:
                    return {"error": "⚠️ Ошибка при получении результатов анализа.\nПопробуйте ещё раз 🥺"}
        else:
            return {"error": "⚠️ Ошибка, не удалось получить ID анализа.\nПопробуйте ещё раз 🥺"}
    else:
        return {"error": f"⚠️ Ошибка запроса VirusTotal.\nПопробуйте ещё раз 🥺"}
...
def check_with_scamadviser(url):
    try:
        ...
        if response.status_code == 200:
            ...
            trustscore_container = soup.find(class_='tile__body tile__body--no-padding d-flex flex-column justify-content-center')
            ...
            return {"error": "⚠️ Ошибка, уровень доверия не найден.\nПопробуйте ещё раз 🥺"}
        else:
            return {"error": "⚠️ Ошибка запроса к ScamAdviser.\nПопробуйте ещё раз 🥺"}
    except Exception as e:
        logger.error(f"Ошибка парсинга ScamAdviser: {e}")
        return {"error": "⚠️ Ошибка парсинга ScamAdviser.\nПопробуйте ещё раз 🥺"}
...
def check_with_google_safe_browsing(url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"error": "⚠️ Ошибка запроса Google Safe Browsing.\nПопробуйте ещё раз 🥺"}
    except Exception as e:
        return {"error": "⚠️ Ошибка при проверке через Google Safe Browsing.\nПопробуйте ещё раз 🥺"}
        ...
        if response.status_code == 200:
            ...
            if title and phone_number in title.text:
                ...
                if "Spam-Risk" in details:
                    details_message += f"Spam-Risk: {details['Spam-Risk']}\n"
                if "Country of origin" in details:
                    details_message += f"Country of origin: {details['Country of origin']}\n"
                if "Most frequently reported" in details:
                    details_message += f"Most frequently reported: {details['Most frequently reported']}\n"
                report_url = url
                if "Spam-Risk" in details or "Most frequently reported" in details:
                    ...
                    return {"is_spam": False, "details": "⚠️ Номер не найден в базе данных спама.", "report_url": report_url}
            else:
                ...
                if analysis_response.status_code == 200:
                    ...
                    if status == "completed":
                        ...
                        if malicious_count > 0:
                            result += "\n\n📝 Итог:\n🗂 Этот файл - Потенциально Вредоносный ⚠️\n❗️ Не рекомендую его открывать ❗️"
                        else:
                            result += "\n\n📝 Итог:\n🗂 Этот файл - Безопасен ✅"
                        return result
                    elif status == "queued":
                        ...
                        return f"⚠️ Ошибка, статус анализа: {status}"
                else:
                    return "⚠️ Ошибка при получении результатов анализа.\nПопробуйте ещё раз 🥺"
        else:
            return f"⚠️ Ошибка запроса VirusTotal: {scan_response.status_code}\nПопробуйте ещё раз 🥺"
    except Exception as e:
        logger.error(f"Ошибка при проверке файла: {e}")
        return "⚠️ Ошибка при загрузке файла.\nПопробуйте ещё раз 🥺"
...
def get_ai_response(query):
    ...
    prompt = f"Отвечай только на вопросы, связанные с кибербезопасностью, мошенничеством и подобными темами. Давай краткие ответы в 2-3 предложения и максимум на 200 слов и символов. Если вопрос не по теме, скажи: 'Извини, я отвечаю только на вопросы про кибербезопасность и мошенничество.' Если вопрос неясен, неполный или я не могу его понять, отвечай только: 'Извини, я не понял твой вопрос. Задай его точнее, пожалуйста!' и не пытайся продолжать или угадывать. Вот вопрос: {query}"
    payload = {
    ...
    try:
        ...
        logger.info(f"Статус ответа: {response.status_code}, Текст: {response.text}")
        ...
        logger.error(f"Ошибка AI/ML API: {e}")
        return "⚠️ Что-то пошло не так с ИИ. Попробуй снова!"
...
def check_message(message):
    ...
    bot.send_message(message.chat.id, "Жду подозрительную ссылку 😊🔗\n⌛️ Ждать около 15 секунд\n\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())

@bot.message_handler(commands=['number'])
def check_phone_message(message):
    user_state[message.chat.id] = 'awaiting_phone'
    bot.send_message(message.chat.id, "Жду от тебя номер 😊☎️\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())

@bot.message_handler(commands=['chat'])
def chat_message(message):
    user_state[message.chat.id] = 'awaiting_chat'
    bot.send_message(message.chat.id, "Задавай вопрос, я тебе отвечу 🤓\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())

@bot.message_handler(commands=['file'])
def check_file_message(message):
    user_state[message.chat.id] = 'awaiting_file'
    bot.send_message(message.chat.id, "Жду от тебя файл для проверки 😊🗂\n⌛️ Ждать 1-4 минуты\n(максимум 20 МБ)\n\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())

@bot.message_handler(func=lambda message: message.text in ["🔗 Проверить ссылку", "☎️ Узнать про номер", "🗂 Проверить файл", "🤓 Задать вопрос ИИ", "👀 Почитать о нас"])
def handle_button(message):
    user_id = message.chat.id
    if message.text == "🔗 Проверить ссылку":
        user_state[user_id] = 'awaiting_link'
        bot.send_message(user_id, "Жду подозрительную ссылку 😊🔗\n⌛️ Ждать около 15 секунд\n\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
    elif message.text == "☎️ Узнать про номер":
        user_state[user_id] = 'awaiting_phone'
        bot.send_message(user_id, "Жду от тебя номер ☺️☎️\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
    elif message.text == "🗂 Проверить файл":
        user_state[user_id] = 'awaiting_file'
        bot.send_message(user_id, "Жду от тебя файл для проверки 😊🗂\n⌛️ Ждать 1-4 минуты\n(максимум 20 МБ)\n\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
    elif message.text == "🤓 Задать вопрос ИИ":
        user_state[user_id] = 'awaiting_chat'
        bot.send_message(user_id, "Задавай вопрос, я тебе отвечу 🤓\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
...
@bot.message_handler(func=lambda message: message.text in ["Отменить⛔️", "/cancel"] and user_state.get(message.chat.id) in ['awaiting_link', 'awaiting_phone', 'awaiting_chat', 'confirming_phone', 'awaiting_file'])
def cancel_action(message):
    reset_state(message.chat.id)
    bot.send_message(message.chat.id, "⛔️ Действие отменено! Что дальше? 😊\nПроверить ссылку - /link 🔗\nУзнать про номер - /number ☎️\nПроверить файл - /file 🗂\nЗадать вопрос ИИ - /chat 🤓\nПочитать о нас - /about 👀", reply_markup=get_main_keyboard())
...
def handle_text_message(message):
    ...
    if state == 'start':
        ...
        if is_valid_link(link):
            ...
            reset_state(user_id)
            bot.send_message(user_id, "😊 Что хочешь сделать дальше?\nПроверить ссылку - /link 🔗\nУзнать про номер - /number ☎️\nПроверить файл - /file 🗂\nЗадать вопрос ИИ - /chat 🤓\nПочитать о нас - /about 👀", reply_markup=get_main_keyboard())
        else:
            bot.send_message(user_id, "Это не похоже на ссылку 🥺\nПопробуй ещё раз или ⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
    elif state == 'awaiting_phone':
        phone_number = message.text
        user_data[user_id] = {'phone_number': phone_number}
        user_state[user_id] = 'confirming_phone'
        bot.send_message(user_id, f"👀 Номер набран правильно? ({phone_number})\n✅ /yes - да\n⛔️ /no - нет", reply_markup=get_confirm_keyboard())
        ...
        if message.text in ["Да✅", "/yes"]:
            ...
            if "error" in result:
                bot.send_message(user_id, result + "\nВведи номер заново или ⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
                user_state[user_id] = 'awaiting_phone'
            else:
            ...
            user_state[user_id] = 'awaiting_phone'
            bot.send_message(user_id, "Введи номер заново ☺️\n⛔️ /cancel - отменить действие", reply_markup=get_cancel_keyboard())
...
@bot.message_handler(content_types=['document', 'photo', 'video'], func=lambda message: user_state.get(message.chat.id) == 'awaiting_file')
